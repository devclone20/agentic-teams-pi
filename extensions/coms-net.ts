...
export default function (pi: ExtensionAPI) {
	...
	// Guards use try/catch so loading coms-net.ts alongside coms.ts is safe:
	// both share flag names and the second registration would otherwise throw.
	try { pi.registerFlag("cname", {
	...
	const onSignal = () => { void cleanShutdown(); };
	pi.on("session_shutdown", async () => {
		// Unhook signals so reloads never stack listeners (or stale closures).
