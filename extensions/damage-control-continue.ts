...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		const total = rules.bashToolPatterns.length + rules.zeroAccessPaths.length + rules.readOnlyPaths.length + rules.noDeletePaths.length;
		ctx.ui.setStatus("damage-control-continue", `🛡️ Damage-Control (continue): ${total} Rules`);
	});
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (violationReason) {
			...
			if (shouldAsk) {
				...
				if (!confirmed) {
					ctx.ui.setStatus("damage-control-violation", `⚠️ Last Violation Blocked: ${violationReason.slice(0, 30)}...`);
				...
				ctx.ui.setStatus("damage-control-violation", `⚠️ Last Violation: ${violationReason.slice(0, 30)}...`);
