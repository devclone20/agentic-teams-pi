...
export default function (pi: ExtensionAPI) {
	...
	async function askForPurpose(ctx: any) {
		// Single ask, not a loop: a cancelled dialog (Esc, or a headless ui that
		// resolves undefined) disables the gate instead of spinning forever.
		...
		if (answer && answer.trim()) {
			...
			ctx.ui.notify("Purpose gate disabled for this session (no purpose given).", "warning");
			gateDisabled = true;
			return;
		}
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// Headless runs (pi -p, --mode json) have a no-op ui whose input()
		// resolves undefined — never gate them.
		if (!ctx.hasUI) return;
	...
	pi.on("input", async (_event, ctx) => {
		if (!ctx.hasUI || gateDisabled) return { action: "continue" as const };
		if (!purpose) {
